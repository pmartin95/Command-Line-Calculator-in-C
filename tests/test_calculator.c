...
#define TEST_ASSERT_MPFR_EQ(actual, expected, message) \
    do { \
        ...
        if (mpfr_cmp_d(abs_diff, MPFR_EPSILON) >= 0) { \
            printf("  ❌ FAIL: %s\n", message); \
            printf("    Expected: "); mpfr_printf("%.15Rf\n", expected); \
            printf("    Actual:   "); mpfr_printf("%.15Rf\n", actual); \
            mpfr_clear(diff); \
            mpfr_clear(abs_diff); \
            return 0; \
        } \
...
int test_expression_mpfr(const char *expr, mpfr_t result) {
    ...
    if (!ast || parser.error_occurred || parser.current_token.type != TOKEN_EOF) {
        free_ast(ast);
        return 0;
    }
...
int test_lexer_functions() {
    ...
    Lexer lexer;
    init_lexer(&lexer, "sin cos tan sqrt log pi e");
...
int test_lexer_scientific_notation() {
    ...
    for (int i = 0; i < 5; i++) {
        Lexer lexer;
        init_lexer(&lexer, tests[i].input);
...
int test_high_precision_parsing() {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        // Parse with our lexer/parser
        int success = test_expression_mpfr(precise_numbers[i], result);
        TEST_ASSERT(success, "Should parse successfully");
        
        // Compare results
        TEST_ASSERT_MPFR_EQ(result, expected, "High precision parsing");
        
        printf("  ✅ %s parsed correctly\n", precise_numbers[i]);
...
int test_high_precision_arithmetic() {
    printf("Testing high precision arithmetic...\n");
    
    // Set higher precision for these tests
    mpfr_prec_t old_precision = global_precision;
    set_precision(256);
...
int main() {
    printf("High-Precision Calculator Test Suite\n");
    printf("====================================\n\n");
    
    // Initialize MPFR with test precision
    ...
    // Lexer tests
    total++; if (test_lexer_basic()) passed++;
    total++; if (test_lexer_functions()) passed++;
    total++; if (test_lexer_scientific_notation()) passed++;
    total++; if (test_high_precision_parsing()) passed++;
    
    // Parser and evaluator tests
    total++; if (test_basic_arithmetic()) passed++;
    total++; if (test_high_precision_arithmetic()) passed++;
    total++; if (test_implicit_multiplication()) passed++;
    total++; if (test_trigonometric_functions()) passed++;
    total++; if (test_logarithmic_functions()) passed++;
    total++; if (test_other_functions()) passed++;
    total++; if (test_comparison_operators()) passed++;
    total++; if (test_complex_expressions()) passed++;
    
    // Precision tests
    total++; if (test_precision_changes()) passed++;
    
    // Error handling tests
    total++; if (test_error_cases()) passed++;
    total++; if (test_domain_errors()) passed++;
